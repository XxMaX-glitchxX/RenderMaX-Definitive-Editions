...
title RenderMaX Definitiva Editions
...
:: --- DETECTAR VERSIÓN DE WINDOWS (Para Módulo Legacy) ---
...
echo.
echo %cCYAN%  ==============================================================================================%cRESET%
echo %cWHITE%                         RENDERMAX Definitive Editions - CONTROL TOTAL%cRESET%
echo %cCYAN%  ==============================================================================================%cRESET%
...
echo     %cYELLOW%[E]%cRESET% OPTIMIZACION EXTREMA AUTOMATICA (Eliminar Bloatware, Privacidad y Servicios)
echo     %cYELLOW%[L]%cRESET% ENTRAR AL SUBMENU "RENDERMAX Para windows 7"
echo.
...
if /i "%ch%"=="E" goto EXTREME_OPT
if /i "%ch%"=="L" goto LEGACY_MENU
...
cls
echo.
echo   %cCYAN%o-------------------------------------------------------------------------o%cRESET%
echo   %cCYAN%^| %cWHITE%RENDERMAX Legacy 7 %cCYAN%                                                      ^|%cRESET%
echo   %cCYAN%o-------------------------------------------------------------------------o%cRESET%
echo   %cCYAN%^| %cWHITE%SISTEMA: Windows %VERSION%    %cCYAN%^| %cWHITE%ESTADO: Administrador    %cCYAN%^| %cWHITE%USUARIO: %username% %cCYAN% ^|%cRESET%
echo   %cCYAN%o-------------------------------------------------------------------------o%cRESET%
echo.
echo       %cCYAN%[1]%cRESET%  %cWHITE%Mantenimiento de Imagen%cRESET%      %cGRAY%(SFC Scan, DISM Health, CheckDisk)%cRESET%
echo       %cCYAN%[2]%cRESET%  %cWHITE%Saneamiento de Almacenamiento%cRESET%  %cGRAY%(WinUpdate, Temp, Prefetch, Logs)%cRESET%
echo       %cCYAN%[3]%cRESET%  %cWHITE%Optimizacion de Red Pro%cRESET%        %cGRAY%(DNS Flush, IP Reset, Winsock)%cRESET%
echo       %cCYAN%[4]%cRESET%  %cWHITE%Debloat ^& System Clean%cRESET%         %cGRAY%(Apps Basura)%cRESET%
echo       %cCYAN%[5]%cRESET%  %cWHITE%Privacidad ^& Telemetria%cRESET%        %cGRAY%(Privacy Flags, DiagTrack, Services)%cRESET%
echo       %cCYAN%[6]%cRESET%  %cWHITE%Personalizacion UI%cRESET%             %cGRAY%(Dark Mode, Classic Menu, Aero)%cRESET%
echo.
echo   %cCYAN%o-------------------------------------------------------------------------o%cRESET%
echo   %cCYAN%^| %cGREEN%[A] EJECUTAR OPTIMIZACION TOTAL%cRESET%   %cYELLOW%[R] RESTAURAR%cRESET%   %cRED%[X] VOLVER AL MENU%cRESET% %cCYAN%^|%cRESET%
echo   %cCYAN%o-------------------------------------------------------------------------o%cRESET%
echo.
echo    %cWHITE%Seleccione una operacion para continuar...%cRESET%

choice /c 123456ARX /n
set "opt=%errorlevel%"

if %opt%==9 goto MENU
if %opt%==8 goto LEGACY_RESTORE
if %opt%==7 goto LEGACY_TOTAL_OPTIMIZATION
if %opt%==6 goto LEGACY_UI_TWEAKS
if %opt%==5 goto LEGACY_PRIVACY_PRO
if %opt%==4 goto LEGACY_DEBLOAT_SYSTEM
if %opt%==3 goto LEGACY_NETWORK_PRO
if %opt%==2 goto LEGACY_STORAGE_CLEAN
if %opt%==1 goto LEGACY_SYS_MAINTENANCE

:LEGACY_SYS_MAINTENANCE
cls
echo  %cCYAN%[*] Iniciando Auditoria de Sistema...%cRESET%
echo.
...
echo N | chkdsk C:
echo.
echo  %cGREEN%[+] Auditoria completada.%cRESET%
if "%LEGACY_AUTO%"=="1" goto :EOF
pause
goto LEGACY_MENU

:LEGACY_STORAGE_CLEAN
cls
echo  %cCYAN%[*] Saneamiento de Almacenamiento...%cRESET%
echo.
...
echo.
echo  %cGREEN%[+] Limpieza profunda finalizada.%cRESET%
if "%LEGACY_AUTO%"=="1" goto :EOF
pause
goto LEGACY_MENU
...
:LEGACY_NETWORK_PRO
cls
echo  %cCYAN%[*] Reconfigurando Stack TCP/IP...%cRESET%
ipconfig /flushdns >nul
netsh winsock reset >nul
netsh int ip reset >nul
echo  %cGREEN%[+] Red optimizada. La conexion se ha refrescado.%cRESET%
if "%LEGACY_AUTO%"=="1" goto :EOF
pause
goto LEGACY_MENU

:LEGACY_DEBLOAT_SYSTEM
cls
echo  %cCYAN%[*] Eliminando Componentes Innecesarios...%cRESET%
...
echo.
echo  %cGREEN%[+] Sistema aligerado.%cRESET%
if "%LEGACY_AUTO%"=="1" goto :EOF
pause
goto LEGACY_MENU
...
:LEGACY_PRIVACY_PRO
cls
echo  %cCYAN%[*] Blindaje de Privacidad...%cRESET%
:: Desactivar Telemetría
reg add "HKLM\Software\Policies\Microsoft\Windows\DataCollection" /v "AllowTelemetry" /t REG_DWORD /d 0 /f >nul
:: Detener servicios de rastreo
sc stop DiagTrack >nul 2>&1
sc config DiagTrack start=disabled >nul 2>&1
echo  %cGREEN%[+] Telemetria y recoleccion de datos desactivada.%cRESET%
if "%LEGACY_AUTO%"=="1" goto :EOF
pause
goto LEGACY_MENU
...
:LEGACY_UI_TWEAKS
cls
echo  %cCYAN%[*] Personalizacion de Interfaz...%cRESET%
if "%VERSION:~0,1%"=="1" (
...
echo.
echo  %cGREEN%[+] Ajustes visuales aplicados.%cRESET%
if "%LEGACY_AUTO%"=="1" goto :EOF
pause
goto LEGACY_MENU
...
:LEGACY_RESTORE
cls
echo  %cYELLOW%[?] Iniciando Proceso de Reversion...%cRESET%
...
echo.
echo  %cGREEN%[OK] Los valores de registro han sido restablecidos.%cRESET%
pause
goto LEGACY_MENU

:LEGACY_TOTAL_OPTIMIZATION
set "LEGACY_AUTO=1"
cls
echo  %cRED%[!] ALERTA: Iniciando optimizacion total del sistema.%cRESET%
echo Esto puede tomar entre 10 a 20 minutos dependiendo de su hardware.
echo.
call :LEGACY_SYS_MAINTENANCE
call :LEGACY_STORAGE_CLEAN
call :LEGACY_NETWORK_PRO
call :LEGACY_DEBLOAT_SYSTEM
call :LEGACY_PRIVACY_PRO
cls
echo  %cGREEN%============================================================%cRESET%
...
:: ========================================================================================
:: --- SECCION: OPTIMIZACION EXTREMA (RENDERMAX V1) ---
:: ========================================================================================

:EXTREME_OPT
cls
echo %cRED%[!] ADVERTENCIA: Se ejecutara la OPTIMIZACION EXTREMA automatica.%cRESET%
echo %cYELLOW%Esto desinstalara aplicaciones predeterminadas, desactivara telemetria profunda,%cRESET%
echo %cYELLOW%servicios en segundo plano, OneDrive, Edge, Copilot y modificara directivas de sistema.%cRESET%
echo.
set /p "confirm=%cCYAN%¿Estas seguro de continuar? (S/N): %cRESET%"
if /i "%confirm%" neq "S" goto MENU

cls
echo %cYELLOW%[+] Iniciando Optimizacion Extrema...%cRESET%
:: Deshabilitar expansion retardada para que los comandos complejos de PowerShell no fallen
...
reg add "HKCU\Software\Software\Policies\Microsoft\InputPersonalization" /v "RestrictImplicitTextCollection" /t REG_DWORD /d 1 /f
...
echo.
echo %cGREEN%[OK] Optimizacion extrema completada.%cRESET%
:: Pausar el proceso
pause
:: Restaurar el entorno anterior
endlocal
:: Salir del proceso y reiniciar explorador
